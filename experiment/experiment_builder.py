...
parser.add_argument('partition', help='partition for slurm', nargs='?')
...
def build_sbatch_file(sbatch_file_dir_path, job_name, identifier, solver,
    ...
    with open(sbatch_file_abs_path, 'w', newline='\n') as slurm_file:
        ...
        slurm_file.write(f'#SBATCH --mem-per-cpu=8G\n')
        slurm_file.write(f'\n')
...
def calculate_partition_from_timeout(partition, timeout):
    m = re.match(r'(?P<H>\d+):.*', timeout)
    hours = int(m['H'])
    if partition is None:
        partition = 'short' if hours <= SHORT_CLUSTER_HOURS else 'long'
    elif partition == 'short' and hours > SHORT_CLUSTER_HOURS:
        partition = 'long'
        warning(f'given timeout {timeout} not match the '
                f'given partition \'short\' '
                f'partition will be changed to \'{partition}\'')
    return partition


def build(name, solvers, flags, networks, property, timeout, partition, cpus_per_task):
    sbatch_files = []
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        for s, (solver, flag) in enumerate(zip(solver_names, flags)):
            for n, network in enumerate(networks):
                ...
                file = build_sbatch_file(
                    ...
                    timeout, partition, cpus_per_task)
        ...
        base_export_dir_name = f'experiment_{name}'
...
def validate_arguments(name, solvers, flags, networks,
                       property, timeout, partition, cpus):
    is_valid = True
    msg = ''
...
def main(args):
    ...
    partition = calculate_partition_from_timeout(args.partition, args.timeout)

    validate_arguments(args.name, solvers, flags, networks,
                       args.property, args.timeout, args.partition, args.cpus)

    build(args.name, solvers, flags, networks, args.property,
          args.timeout, partition, args.cpus)
